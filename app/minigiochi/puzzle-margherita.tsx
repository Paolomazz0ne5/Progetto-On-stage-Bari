...
export default function PuzzleMargheritaScreen() {
  ...
  // Double-tap per riportare pezzo al vassoio
  const lastTapRef = useRef<{ pieceId: number; time: number } | null>(null);

  const handlePieceDoubleTap = (pieceId: number, fromGridIdx: number | null) => {
    if (fromGridIdx === null || FIXED_PIECES.includes(fromGridIdx)) return;

    const now = Date.now();
    const lastTap = lastTapRef.current;

    if (lastTap && lastTap.pieceId === pieceId && now - lastTap.time < 300) {
      // È un double tap!
      Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);

      // Riporta il pezzo al vassoio
      const newGrid = [...gridPieces];
      newGrid[fromGridIdx] = null;
      setGridPieces(newGrid);

      setTrayPieces((prev) => [...prev, pieceId]);
      setSelectedPiece(null);

      lastTapRef.current = null;
    } else {
      // Primo tap o tap su pezzo diverso
      lastTapRef.current = { pieceId, time: now };
      handlePieceTap(pieceId, fromGridIdx, false);
    }
  };

  // Clicca su una casella della griglia
  const handleGridCellTap = (cellIdx: number) => {
    if (FIXED_PIECES.includes(cellIdx)) return;

    const existingPieceId = gridPieces[cellIdx];

    // Se non hai selezionato niente
    if (!selectedPiece) {
      // Se la casella ha un pezzo, selezionalo
      if (existingPieceId !== null) {
        handlePieceTap(existingPieceId, cellIdx, false);
      }
      return;
    }

    // Se hai un pezzo selezionato:
    // Se clicchi la stessa casella dove è il pezzo selezionato, deselezionalo
    if (selectedPiece.fromGridIdx === cellIdx) {
      setSelectedPiece(null);
      return;
    }

    // Altrimenti piazzalo (potrebbe scambiare se la cella è occupata)
  ...
  const handlePlacement = (
    ...
    if (existingPieceId === null) {
      ...
      // Cella occupata: non puoi piazzare!
      Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);
      Alert.alert('Casella occupata', 'Seleziona una casella vuota o rimuovi il pezzo che c\'è qui (doppio tap su di esso).');
    }
  };
              ...
              {Array.from({ length: 16 }).map((_, idx) => {
                ...
                return (
                  ...
                  >
                    {pieceId !== null ? (
                      <Pressable
                        onPress={() => !isLocked && handlePieceDoubleTap(pieceId, idx)}
                        style={[
                          styles.placedTile,
